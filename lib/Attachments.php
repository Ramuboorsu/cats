...
class Attachments
{
    ...
    //adding count varible for foldure
    ...
    public function setDirectoryName($attachmentID, $directoryName)
    {
     ...
     /**
     * Developer-1 Sets the file name in which an attachment is contained. Directory
     * names should be relative to attachments/.
     *
     * @param integer Attachment ID.
     * @param string Directory name (under attachments/).
     * @return boolean True if successful; false otherwise.
     */
    public function setFileName($attachmentID, $filename)
    {
        $sql = sprintf(
            "UPDATE
                attachment
...
class AttachmentCreator
{
 ...
 /* Developer-1 Changed function by adding two more aruguments firstname,lastname
For attaching resume to already exisist candidate */
    public function createFromUpload($dataItemType, $dataItemID, $fileField,
        $isProfileImage, $extractText ,$firstname ,$lastname,$email)
    {
...
For attaching resume to already exisist candidate */
        return $this->createGeneric(
            ...
            $originalFilename, $tempFilename, $contentType, false, true ,$firstname ,$lastname,$email
        );
    }
    ...
    /* Developer-1 Changed function by adding two more aruguments firstname,lastname
        For adding Reume to database and to path*/
    public function createFromFile($dataItemType, $dataItemID, $filePath,
        $title, $contentType, $extractText, $fileExists,$firstname,$lastname,$email)
    {

        // echo '<script>alert("createFromFile'.$dataItemID.'");</script>';
...
For attaching resume to already exisist candidate */
        return $this->createGeneric(
            ...
            $fileExists,$firstname,$lastname,$email
        );
    }
...
For  */
        ...
        $contentType, $fileContents, $fileExists,$firstname,$lastname,$email)
    {
        ...
        if ($fileExists && !$fileContents)
        {
             ...
             * duplicate occurred.
             */
//RAmu Code Edited for duplicate

            if (!empty($duplicates))
            {

        /* Developer-1 User Adds another Resume it should add to his Folder */        
               
            //     echo '<script>alert("'.$dataItemType.','.$dataItemID.','.$fileSize.','.$md5sum.','.count($duplicates).'");
            // </script>';
            //Developer-1 Db Connection
                // $db = DatabaseConnection::getInstance();
                $this->_duplicatesOccurred = true;
//         $rr = "ramu";
//                 // $sel = $db->query(sprintf('select * from attachment '
//                 //                 . 'WHERE data_item_id = %d',32
//                 //             ));
//                 $sql = sprintf(
//             "SELECT
//                 *
...
//                 foreach($rs as $set)
//                 {
//                     $fet = $set['original_filename'];
//                     $attachmentID = $set['attachment_id'];
//                 }
//                 $org = $fet.",".$storedFilename ;

//                 $db->query(sprintf('UPDATE attachment SET original_filename ="%s",stored_filename="%s"'.
//                                  ' WHERE data_item_id = "%s"',$org,$org,$dataItemID
//                             ));

                  // echo '<script>alert("before adding'.$dataItemID.'");</script>';
      ...
      if($attachmentID > 0)
      {
  // echo '<script>alert("after adding'.$dataItemID.'");</script>';

   $uniqueDirectory = $this->_createDirectory(
            $attachmentID, $storedFilename,$firstname,$lastname,$email,$dataItemID
        );
        if (!$uniqueDirectory)
        {
...
// echo '<script>alert("hello+'.$uniqueDirectory.'");</script>';
        /*Developer-1  Create the full path name to the file. */
        $newFileFullPath1 = $uniqueDirectory . $storedFilename;
        if(file_exists($newFileFullPath1))
        {
  //count of file        
$filecnt = sprintf(
            "SELECT
            ...
            FROM
                attachment
            WHERE
              stored_filename ='%s' and data_item_id=%d",
            $storedFilename,$dataItemID
        );
$filers = $this->_db->getAllAssoc($filecnt);
foreach ($filers as $file) {
    $tot = $file['cnt'];

}
     // $storedFileName1 ="(".$attachmentID.")".$storedFilename;
     $storedFileName1 ="(".$attachmentID.")".$storedFilename;
          $newFileFullPath = $uniqueDirectory . $storedFileName1;
          $attachments->setFileName($attachmentID,$storedFileName1);
        }
        else
        {
           $newFileFullPath = $uniqueDirectory . $storedFilename;
            ...
            //         echo '<script>alert("if dublicate remove file from  massimport folder");
            // </script>';
                   // unlink($tempFilename);
                }
            }
...
// echo '<script>alert("'.$uniqueDirectory.'");</script>';
        ...
        if ($fileContents !== false)
        {
            ...
            if (!empty($duplicates))
            {
                // echo '<script>alert("enter to dup");</script>';
                if (!@copy($tempFilename, $newFileFullPath))
                {
        ...
        else if ($fileExists)
        {
            ...
            // echo '<script>alert("deleting files from temp file massimport file");
            // </script>';
    ...
    private function _createDirectory($attachmentID, $storedFilename,$firstname,$lastname,$email,$dataItemID)
    {

         $this->_db = DatabaseConnection::getInstance();

 // echo '<script>alert("_createDirectory path'.$dataItemID.'");</script>';
        //Developer-1 $cnt for folders count if first and last names are same
        $cnt =1;
...
{
    //Developer-1 checking for duplicate folder of diff mail id's
    // $db = DatabaseConnection::getInstance();
    $basePath ='/';
    $name = $firstname."_".$lastname;
    file_exists($basePath . $name);
    // echo '<script>alert("'.$IDGroupDirectory.'");</script>';
        $uniqueDirectory1 = sprintf(
            '%s/%s/',
            $IDGroupDirectory,
           $name
        );

      // echo '<script>alert("unique-'.$uniqueDirectory1.'");</script>';
...
$rs = $this->_db->getAllAssoc($sqlcnt);
foreach ($rs as $sqlcnt) {
    $totcnt1 = $sqlcnt['cnt'];
...
if($totcnt1 == 1)
{
$replace = str_replace('./attachments/', '', $uniqueDirectory1);
$sql = sprintf(
            "SELECT
                count(*) as cnt
            FROM
                attachment
            WHERE
                 directory_name ='%s'",$replace
        );

         // echo '<script>alert("ram-'.$replace.'");</script>';
         // echo '<script>alert("ram-'.$cnt.'");</script>';
$rs = $this->_db->getAllAssoc($sql);
foreach ($rs as $getcnt) {
    $totcnt = $getcnt['cnt'];

}

//Developer-1 checking directory name exisist or not if not create
if($totcnt == 0)
{

    $uniqueDirectory =$uniqueDirectory1;
}
//Developer-1 if directory exisist cheking same user or not
else
{
...
$sql = sprintf(
            ...
            FROM
                attachment
            WHERE
                 data_item_id ='%s'",$dataItemID
        );

$rs = $this->_db->getAllAssoc($sql);
foreach ($rs as $getcnt) {
    $totcnt = $getcnt['cnt'];
//     echo '<script>alert("dataItemID-'.$dataItemID.'");</script>';
// echo '<script>alert("dataItemID-'.$totcnt.'");</script>';
}


//Developer-1 when 1st time data inserted in attachments
if($totcnt == 1) 
{

    // echo '<script>alert("when 1st time data inserted in attachments");</script>';
$replace = str_replace('./attachments/', '', $uniqueDirectory1);
$sql = sprintf(
            "SELECT
                count(*) as cnt
...
//Developer-1 if attachment available more than 1st time
elseif($totcnt > 1)
{
    // echo '<script>alert("total count > 1-'.$totcnt.'");</script>';
    // echo '<script>alert("when more than 1st time data inserted in attachments-'.$dataItemID.'");</script>';
$sql = sprintf('SELECT * FROM  attachment WHERE data_item_id = %d ORDER BY attachment_id  ASC',$dataItemID);

     // echo '<script>alert("sql'.$sql.'");</script>';
    $rsss = $this->_db->getAllAssoc($sql);
foreach ($rsss as $getcntss) {
    // print_r('<script>alert("final path foreach-'.$getcntss.'");</script>');
    $path = $getcntss['directory_name'];
    break;
    // echo '<script>alert("'.$getcntss['directory_name'].'");</script>';
     }

   $uniqueDirectory ="./attachments/".$path; 
 // echo '<script>alert("final path'.$uniqueDirectory.'");</script>';
}
else
{
    $uniqueDirectory =$uniqueDirectory1;
