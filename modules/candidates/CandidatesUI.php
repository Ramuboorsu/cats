...
class CandidatesUI extends UserInterface
{
    ...
    private function onAddEditImage()
    {
        ...
        $attachmentCreator->createFromUpload(
            DATA_ITEM_CANDIDATE, $candidateID, 'file', true, false ,$firstName,$lastName,$email1
        );
...
//Developer-1 ramu geting data from candidates table for attaching file to candidate
        $sql = sprintf(
            'SELECT
                *
            FROM
...
foreach($rs as $set)
        ...
        $attachmentCreator = new AttachmentCreator($this->_siteID);
        $attachmentCreator->createFromUpload(
            DATA_ITEM_CANDIDATE, $candidateID, 'file', false, $isResume ,$firstName ,$lastName,$email1
        );
    ...
    private function _addCandidate($isModal, $directoryOverride = '')
    {
        ...
        else if (isset($_FILES['file']) && !empty($_FILES['file']['name']))
        {
            ...
            $attachmentCreator = new AttachmentCreator($this->_siteID);
            $attachmentCreator->createFromUpload(
                DATA_ITEM_CANDIDATE, $candidateID, 'file', false, true ,$firstName,$lastName,$email1
            );
        ...
        else if (LicenseUtility::isParsingEnabled())
        {
            ...
            if ($tempFile !== false && $tempFullPath !== false)
            {
                ...
                $attachmentCreator->createFromFile(
                    DATA_ITEM_CANDIDATE, $candidateID, $tempFullPath, $tempFile, '', true, true,$firstName,$lastName,$email1
                );
