        ...
        for ($ind=0; $ind<count($_SESSION['CATS_PARSE_TEMP']); $ind++)
        {
            ...
            if (isset($doc['success']) && $doc['success'])
            {
                ...
                if (isset($doc['lastName']) && $doc['lastName'] != '' && isset($doc['firstName']) && $doc['firstName'] != '')
                {
                    ...
                    //Developer-1 changing email position to last it should be unique
                    ...
                    if ($isCandidateUnique)
                    {
                        ...
                        if ($candidateID > 0)
                        {
                            ...
                            if ($ac->createFromFile(DATA_ITEM_CANDIDATE, $candidateID, $doc['name'], $doc['realName'],
                                '', true, true,$doc['firstName'],$doc['lastName'],$doc['email']))
            //                     echo '<script>alert("candidate added");
            // </script>';
                            {
                    ...
                    {
//Developer-1 If candidate is not unique collect details and send 
            ...
            FROM
                candidate
            WHERE
                 (email1="%s" or email2="%s" )',$doc['email'],$doc['email']
        );       
$rs = $db->getAllAssoc($sql);
foreach($rs as $set)
                {
                    $candidateID = $set['candidate_id'];
                    ...
                    $lastname = $set['last_name'];
                        ...
                        $ac = new AttachmentCreator($siteID);
                            if ($ac->createFromFile(DATA_ITEM_CANDIDATE, $candidateID, $doc['name'], $doc['realName'],
                                '', true, true,$doc['firstName'],$doc['lastName'],$doc['email']))
                               {
            //             echo '<script>alert("duplicate candidate");
                ...
                if (!$candidateAdded)
                {
                    ...
                    if (!$brExists)
                    {
                        ...
                        $attachmentCreator->createFromFile(
                            DATA_ITEM_BULKRESUME, 0, $doc['name'], $doc['realName'], '', true, true,$doc['firstName'],$doc['lastName'],$doc['email']
                        );

                        if ($attachmentCreator->isError())
                        {
            //                 echo '<script>alert("isError");
            // </script>';
                            $error = true;
                        }
                        if ($attachmentCreator->duplicatesOccurred())
                        {
            //             echo '<script>alert("duplicatesOccurred");
            // </script>';
                            $error = true;
                        }
                    }
